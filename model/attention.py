...
class NonLocalSparseAttention(nn.Module):
    ...
    def LSH(self, hash_buckets, x):
        ...
        random_rotations = torch.randn(rotations_shape, dtype=x.dtype, device=device).expand(N, -1, -1, -1) #[N, C, n_hashes, hash_buckets//2]旋转
    ...
    def add_adjacent_buckets(self, x):#处理相邻桶
    ...
    def forward(self, input):
        ...
        x_match = F.normalize(x_att_buckets, p=2, dim=-1,eps=5e-5)#除以一个范数
                #5e-5  默认1e-12
        #允许处理相邻的桶
        ...
        #attention
        ret = torch.einsum('bukij,bukje->bukie', score, y_att_buckets) #[N, n_hashes, num_chunks, chunk_size, C] 求和
